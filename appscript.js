...
const doGet = (e) => {
    try {
        ...
        if (!action || !row || !approver) {
            return createResponsePage({ title: 'Parameter Tidak Lengkap', message: 'Aksi tidak dapat diproses karena parameter (action, row, approver) tidak lengkap pada URL.', themeColor: '#dc3545', icon: '⚠' });
        }
        ...
        try {
            ...
            if (statusColIndex === -1) {
                return createResponsePage({ title: 'Kesalahan Konfigurasi', message: `Kolom status dengan nama "${STATUS_COLUMN_NAME}" tidak dapat ditemukan di sheet "${DATA_ENTRY_SHEET_NAME}".`, themeColor: '#dc3545', icon: '⚙️' });
            }
            if (parseInt(row) > SHEET.getLastRow()) {
                return createResponsePage({ title: 'Tindakan Sudah Diproses', message: 'Permintaan ini sepertinya sudah diproses atau baris data tidak lagi ditemukan.', themeColor: '#ffc107', icon: 'ⓘ' });
            }
            ...
            if (action === 'approve') {
                ...
                try {
                    sendApprovalNotificationEmail(dataFromRowObj, approver);
                } catch (emailError) {
                ...
                SHEET.getRange(parseInt(row), statusColIndex + 3).setValue(new Date());
                try {
                    sendRejectionNotificationEmail(dataFromRowObj, approver);
                } catch (emailError) {
                    console.error("Gagal mengirim email notifikasi penolakan:", emailError.toString(), emailError.stack);
                }
                return createResponsePage({ title: 'Permintaan Ditolak', message: 'Status permintaan telah diperbarui menjadi <strong>Ditolak</strong> dan notifikasi revisi telah dikirim.', themeColor: '#dc3545', icon: '✖' });
            } else {
                return createResponsePage({ title: 'Aksi Tidak Valid', message: `Aksi "${action}" tidak dikenali oleh sistem.`, themeColor: '#dc3545', icon: '⚠' });
            }
        } finally {
            lock.releaseLock();
        }
        ...
        return createResponsePage({ title: 'Terjadi Kesalahan Internal', message: `Maaf, terjadi kesalahan. Silakan hubungi administrator.<br><small>Detail: ${error.message}</small>`, themeColor: '#dc3545', icon: '⚠' });
    }
};
...
function populateHtmlTemplate(data, approvalDetails = null) {
    // --- Langkah 1: Persiapan & Pengelompokan Data ---
    ...
    // --- Langkah 2: Bangun HTML untuk Tabel dan Grand Total ---
    ...
    // --- Langkah 3: Rakit Template HTML Final ---
